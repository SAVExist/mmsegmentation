...
# weight = 1 / (class_frequency + epsilon)
# Для 90%/5%/5%: примерно [0.37, 6.67, 6.67]

Для вашего точного дисбаланса 90.8% / 5.0% / 4.2% вот расчеты весов классов:

Вариант 1: Медианная нормализация (рекомендуется для старта)
Наиболее стабильный подход, не дающий слишком агрессивных весов:
...
На данном этапе сформулированы и протестированы две стартовые гипотезы (бейзлайны) для задачи мультиклассовой семантической сегментации кошек и собак. Обе модели используют одинаковый датасет, но различаются архитектурой, аугментациями и стратегией обучения.
...
**Модель**: UNet с ResNet-34 в качестве backbone  
**Лосс**: `CrossEntropyLoss` (стандартный)  
...
- ResNet-34 — баланс между глубиной и скоростью, предобучен на ImageNet.
...
✅ Цель: получить **базовую модель**, не переобучившуюся, с mDice > 0.65.

### Результаты обучения

- 🔗 **Конфиг**: [`configs/exp_unet_r34.py`](../configs/exp_unet_r34.py)
