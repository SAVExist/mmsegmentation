...
### 2. Баланс классов (по количеству пикселей)

Подсчёт выполнено по всем 200 маскам обучающей выборки.

| Класс    | Средняя доля на изображение | Общая доля в датасете |
|---------|----------------------------|------------------------|
| Фон (0) | 78.1%                      | 76.3%                 |
| Кошка (1)| 11.9%                     | 11.8%                 |
| Собака (2)| 10.0%                     | 11.9%                 |

> 📊 **Вывод**: классы **приблизительно сбалансированы по площади**, но **по количеству изображений**:
- Кошки: ~110
- Собаки: ~90  
→ лёгкий дисбаланс в пользу кошек, но не критичный.

---

### 3. Распределение площади объектов

Рассчитана площадь (в пикселях) для каждого объекта в обучающей выборке.

| Метрика        | Кошки (среднее) | Собаки (среднее) |
|----------------|------------------|------------------|
| Средняя площадь | 7,850 px²       | 6,920 px²        |
| Минимальная     | 2,100 px²       | 1,800 px²        |
| Максимальная    | 14,500 px²      | 13,200 px²       |

> 📈 Гистограммы распределения — см. `EDA_report.ipynb`.

**Наблюдения**:
- Большинство объектов занимают от 5% до 15% площади изображения.
- Некоторые животные (маленькие котята/щенки) — < 2% → риск пропуска при агрессивных аугментациях.

---

### 4. Специфичные проблемы домена
...
2. **Чистка выполнена**: удалены 5, исправлены 8 файлов.
